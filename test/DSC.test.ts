...
describe('Erc20WithFees', () => {
	...
	describe('Balance', () => {
		it('shows correct balance', async () => {
			const { ERC20WithFees, deployer, users, decimals, feeRate } =
				await setup()
			...
			const balanceBeforeWithFees = await ERC20WithFees.balanceOfWithFee(
				user.address,
			)
			...
			const balanceWithFeesAfter = await ERC20WithFees.balanceOfWithFee(
				user.address,
			)
		...
		it('shows correct balance: dust', async () => {
			...
			const balanceWithFees = await ERC20WithFees.balanceOfWithFee(
				user.address,
			)
	...
	describe('Transfer', () => {
		it('always possible to transfer all tokens', async () => {
			...
			let balanceBefore = await ERC20WithFees.callStatic.balanceOf(
				sender.address,
			)

			await sender.ERC20WithFees.transferAll(receiver.address)

			let balance = await ERC20WithFees.callStatic.balanceOf(
				sender.address,
			)
			let balanceAndFee = await ERC20WithFees.callStatic.balanceOfWithFee(
				sender.address,
			)
			...
			let receiverBalance = await ERC20WithFees.callStatic.balanceOf(
				receiver.address,
			)
			let receiverFee = balanceAndFee.sub(balance)

			expect(receiverBalance.toNumber()).to.be.closeTo(
	...
	describe('Fee last paid', () => {
		it('requires amount > 0', async () => {
			...
			await expect(
				sender.ERC20WithFees.transfer(receiver.address, amount),
			).to.be.revertedWith(
	...
	describe('Fee collection', () => {
		it('trigger fee deduction', async () => {
			const { ERC20WithFees, minter, feeCollector, users, decimals } =
				await setup()

			const user = users[0]

			await fundFromDeployer(
				minter.ERC20WithFees,
				user.address,
			...
			await expect(user.ERC20WithFees.collectFees([user.address]))
				.to.emit(ERC20WithFees, 'Transfer')
				.withArgs(user.address, feeCollector.address, fee)
		})
		it('collects 100% fees', async () => {
			...
			let necessaryTime = BigNumber.from(10 ** decimals)
				.div(feeRate)
				.mul(365 * 24 * 60 * 60)
				.add(1)
			...
			const balance = await ERC20WithFees.balanceOf(user.address)
			const balanceWithFee = await ERC20WithFees.balanceOfWithFee(
				user.address,
			)
			...
			await expect(user.ERC20WithFees.collectFees([user.address]))
				.to.emit(ERC20WithFees, 'Transfer')
				.withArgs(user.address, feeCollector.address, amount)

			const balanceAfterCollection = await ERC20WithFees.balanceOf(
				user.address,
			)
			const balanceWithFeeAdterCollection =
				await ERC20WithFees.balanceOfWithFee(user.address)
	...
	describe('Calculate fees', () => {
		it('fees deducted from balance correctly', async () => {
			...
			expect(await ERC20WithFees.balanceOfWithFee(user.address)).to.equal(
				0,
			)
			...
			const balance = await ERC20WithFees.callStatic.balanceOf(
				user.address,
			)
			const balanceAndFee =
				await ERC20WithFees.callStatic.balanceOfWithFee(user.address)
		...
		it('fees from dust amounts', async () => {
			...
			const balanceAndFee = await ERC20WithFees.balanceOfWithFee(
				user.address,
			)
	...
	describe('Burn', () => {
		it('only by minter role', async () => {
			...
			await expect(
				user.ERC20WithFees.burn(user.address, amount),
			).to.be.revertedWith(
		...
		it('needs allowance', async () => {
			...
			await expect(
				minter.ERC20WithFees.burn(user.address, amount),
		...
		it('burns tokens', async () => {
			...
			expect(balanceAfter.toNumber()).to.be.closeTo(
				balanceBefore.sub(burnAmount).toNumber(),
				1 * 10 ** decimals,
			)
	...
	describe('Mint', () => {
		it('only minter', async () => {
			...
			await expect(
				users[0].ERC20WithFees.mint(amount),
			).to.be.revertedWith(
				'ERC20WithFees: only minter can call this function',
			)
		})
		...
		it('mints to minter address', async () => {
			...
			const balanceBefore = await feeCollector.ERC20WithFees.balanceOf(
				minter.address,
			)
			...
			const balanceAfter = await feeCollector.ERC20WithFees.balanceOf(
				minter.address,
			)
	...
	describe('Fee exemption', () => {
		it('exempt addresses do not pay fees', async () => {
			...
			const balanceBefore = await ERC20WithFees.balanceOf(
				feeCollector.address,
			)
			const balanceWithFeeBefore = await ERC20WithFees.balanceOfWithFee(
				feeCollector.address,
			)
			...
			const balanceAfter = await ERC20WithFees.balanceOf(
				feeCollector.address,
			)
			const balanceWithFeeAfter = await ERC20WithFees.balanceOfWithFee(
				feeCollector.address,
			)
	...
	describe('set new fee collection address', () => {
		it('only owner', async () => {
			...
			await expect(
				user.ERC20WithFees.setFeeCollectionAddress(user.address),
			).to.be.reverted
		})
		...
		it('Cannot be zero address', async () => {
			const { ERC20WithFees } = await setup()
			await expect(
				ERC20WithFees.setFeeCollectionAddress(
		...
		it('Set new fee collector', async () => {
			const { ERC20WithFees, users, deployer, feeCollector } =
				await setup()

			const user = users[0]
			await expect(
				deployer.ERC20WithFees.setFeeCollectionAddress(user.address),
			).to.not.be.reverted

			const lastPaid = await ERC20WithFees.feeLastPaid(
				feeCollector.address,
			)
			const latestBlockTime = await time.latest()
	...
	describe('Set new minter address', () => {
		it('only owner', async () => {
			...
			await expect(user.ERC20WithFees.setMinterRole(user.address)).to.be
				.reverted
		})
		...
		it('cannot be zero address', async () => {
			const { ERC20WithFees } = await setup()
			await expect(
				ERC20WithFees.setMinterRole(ethers.constants.AddressZero),
			).to.be.revertedWith(
				'ERC20WithFees: collection address cannot be zero',
			)
		})
		...
		it('set up new minter role', async () => {
			...
			const user = users[0]
			await expect(deployer.ERC20WithFees.setMinterRole(user.address)).to
				.not.be.reverted
	...
	describe('Set new fee rate', () => {
		it('Only owner', async () => {
			...
			await expect(
				user.ERC20WithFees.setFeeRate(ethers.utils.parseUnits('1', 8)),
			).to.be.reverted
		})
		...
		it('Cannot be more than max', async () => {
			...
			await expect(
				ERC20WithFees.setFeeRate(max.add(1)),
			).to.be.revertedWith(
		...
		it('delay ', async () => {
			...
			await expect(
				deployer.ERC20WithFees.setFeeRate(fee.add(1)),
		...
		it('Set new fee rate', async () => {
			...
			await expect(deployer.ERC20WithFees.setFeeRate(fee.add(1))).to.not
				.be.reverted
			let feeLastChangedAfter = await ERC20WithFees.lastFeeChange()
		...
		it('forgets outstanding debt with old fee rate', async () => {
			...
			let balancBefore = await ERC20WithFees.callStatic.balanceOf(
				user.address,
			)
			...
			let balanceAfter = await ERC20WithFees.callStatic.balanceOf(
				user.address,
			)
	...
	describe('Oracle', () => {
		...
		it('cannot be zero address', async () => {
			const { ERC20WithFees } = await setup()
			await expect(
				ERC20WithFees.setOracleAddress(ethers.constants.AddressZero),
		...
		it('only owner', async () => {
			...
			await expect(user.ERC20WithFees.setOracleAddress(user.address)).to
				.be.reverted
		})
		...
		it('can be changed', async () => {
			...
			await expect(ERC20WithFees.setOracleAddress(user.address))
				.to.emit(ERC20WithFees, 'OracleAddressChanged')
		...
		it('limits token minting if set', async () => {
			...
			await expect(ERC20WithFees.mint(max.add(1))).to.be.revertedWith(
	...
	describe('Whitelist addresses', () => {
		it('non owner reverts', async () => {
			...
			await expect(user.ERC20WithFees.setFeeExempt(user.address)).to.be
				.reverted

			await expect(user.ERC20WithFees.unsetFeeExempt(user.address)).to.be
				.reverted
		})
		...
		it('owner can whitelist', async () => {
			...
			await expect(ERC20WithFees.setFeeExempt(user.address)).to.not.be
				.reverted

			isExempt = await ERC20WithFees.feeExempt(user.address)
			assert.equal(isExempt, true)

			await expect(ERC20WithFees.unsetFeeExempt(user.address)).to.not.be
				.reverted
	...
	describe('Test Upgrade', () => {
		...
		it('Upgrade only by owner', async () => {
			...
			await expect(user.ERC20WithFees.upgradeTo(user.address)).to.be
				.reverted
		})
